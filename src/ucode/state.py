...
def build_agent_state(state: dict) -> dict[str, dict]:
    ...
    # Cross-platform credential helper: a plain command string for agents that
    # shell out (Claude/pi) and an argv list for those that exec directly (Codex).
    auth_command = build_auth_helper_string(workspace, profile)
    auth_argv = build_auth_helper_argv(workspace, profile)
